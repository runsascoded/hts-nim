...
RUN mkdir -p /usr/local/include \
 && git clone --depth 1 -b v1.14 https://github.com/ebiggers/libdeflate.git \
 && cd libdeflate \
...
RUN git clone --depth 1 -b 1.15 --recursive https://github.com/samtools/htslib \
 && cd htslib \
 && autoheader && autoconf \
 && ./configure --enable-s3 --enable-gcs --enable-libcurl --with-libdeflate \
 && make -j4 CFLAGS="-fPIC -O3" install

RUN git clone --depth 1 -b 1.15 --recursive https://github.com/samtools/bcftools \
